<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 nav-container">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <!-- Right Side (Log in / Register or User Dropdown) -->
            <div class="hidden sm:flex sm:items-center sm:ms-6 space-x-4">
                @auth
                    @if(Auth::user())
                        <x-dropdown align="right" width="48" class="dropdown-menu">
                            <x-slot name="trigger">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                ...
                @if(auth()->user() && auth()->user()->is_admin)
                    <x-responsive-nav-link :href="route('admin.dashboard')" :active="request()->routeIs('admin.dashboard')" class="nav-link">
                        {{ __('Admin Dashboard') }}
                    </x-responsive-nav-link>
                @endif
            @endauth
            @guest
                <x-responsive-nav-link :href="route('login')" :active="request()->routeIs('login')" class="nav-link">
                    {{ __('Log in') }}
                </x-responsive-nav-link>
                @if (Route::has('register'))
                    <x-responsive-nav-link :href="route('register')" :active="request()->routeIs('register')" class="nav-link">
                        {{ __('Register') }}
                    </x-responsive-nav-link>
                @endif
            @endguest
